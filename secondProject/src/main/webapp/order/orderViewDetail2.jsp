...
<%-- 	<c:set var="i" value="${0 }"/> --%>
	...
	<tr height=80>
		...
		<td width=110 style="text-align:center;border-bottom:1px solid #5055B1;">${ca.price1 }</td>
		<td width=110 style="text-align:center;border-bottom:1px solid #5055B1;">${ca.qty * ca.price1 }</td>
		<td width=100 style="text-align:center;border-bottom:1px solid #5055B1;"></td>
		...
		<td width=250 rowspan=3 border=0 class="fix1">
					...
					<td style="vertical-align:top;">
						<table border=0 width=350 style="border:2px solid #8D0837;vertical-align:top;" height=150>
							...
							<tr>
								...
								<td></td>
								<td style="text-align:right;">
								<c:if test="${o.od_price < cartSum && o.od_price >=1 }">${3000 } 원&nbsp;&nbsp;</c:if>		
								<c:if test="${o.od_price >= cartSum}">무료&nbsp;&nbsp;</c:if>
			...
			<table style="" height=50 width=350>
				...
				<tr>
					...
					<c:if test="${o.od_status eq '주문' }"><td width=175 style="text-align:center;"><input style="width:90%;height:40px;background-color:#767676;color:#FEFEFE;border-radius:15px;border:0px;font-size:18px;" type="button" value="주문취소" onClick="order_cancel2()"></td></c:if>
					</c:when>
					<c:otherwise>
...
function order_cancel(){
	...
	if(result==true){
		location.href="/order/cancel?od_id="+od_id+"&order_option=a";
		location.href="/member/my_page?option=order";
	}
}
</script>