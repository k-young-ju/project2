...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
		...
		<table border=0 width=1200 align=center>
				...
				<c:if test="${startPage > pageBlock }">
					<a href="javascript:callFunction4(${startPage - pageBlock },'sellUsed');">[이전]</a>
				</c:if>
				</td>
...
<%-- 							<a href="javascript:callFunction('mileage','new',${i });"><span style="padding:0px 4px;"><font color=red><b>${i }</b></font></span></a> --%>
							<a href="javascript:callFunction4(${i },'sellUsed');"><span style="padding:0px 4px;"><font color=red><b>${i }</b></font></span></a>
						</c:when>
						<c:otherwise>
...
<%-- 							<a href="javascript:callFunction('mileage','new',${i });"><span style="padding:0px 4px;">${i }</span></a> --%>
							<a href="javascript:callFunction4(${i },'sellUsed');"><span style="padding:0px 4px;">${i }</span></a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				</td>			
				...
				<td width=75 style="text-align:center">
				<c:if test="${endPage < pageCount }">
					<a href="javascript:callFunction4(${startPage + pageBlock },'sellUsed');">[다음]</a>
				</c:if>
				</td>
			</c:if>
				<tr>
