...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
		...
		<table border=0 width=1200 align=center>
				...
				<c:if test="${startPage > pageBlock }">
					<a href="javascript:callFunction(${startPage - pageBlock },'productUsed');">[이전]</a>
				</c:if>
				</td>
...
<%-- 							<a href="javascript:callFunction('mileage','new',${i });"><span style="padding:0px 4px;"><font color=red><b>${i }</b></font></span></a> --%>
							<a href="javascript:callFunction(${i },'productUsed');"><span style="padding:0px 4px;"><font color=red><b>${i }</b></font></span></a>
						</c:when>
						<c:otherwise>
...
<%-- 							<a href="javascript:callFunction('mileage','new',${i });"><span style="padding:0px 4px;">${i }</span></a> --%>
							<a href="javascript:callFunction(${i },'productUsed');"><span style="padding:0px 4px;">${i }</span></a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				</td>			
				...
				<td width=75 style="text-align:center">
				<c:if test="${endPage < pageCount }">
					<a href="javascript:callFunction(${startPage + pageBlock },'productUsed');">[다음]</a>
				</c:if>
				</td>
			</c:if>
				<tr>
					...
					<table width=1200 border=0 style="text-align: center">
						...
						<tr height=20><td colspan=8 style="border-bottom:2px solid #5055B1;"/></tr>
				
					<c:forEach var="list" items="${v2 }">
						<tr style="text-align: center" onclick="location.href='/board/view?uid=${list.uid}&list_option=3'">
							...
							<td width=300 style="border-bottom:2px solid #5055B1;">${list.n_book }</td>
							<td width=200 style="border-bottom:2px solid #5055B1;"><fmt:formatNumber value="${list.price1 }"/></td>
							<td width=100 style="border-bottom:2px solid #5055B1;">${list.id }</td>
							<td width=200 style="border-bottom:2px solid #5055B1;">${list.signdate }</td>
							<td width=50 style="border-bottom:2px solid #5055B1;">
								<a href="/item/modify_used?uid=${list.uid}&pageNum=${pageNum}&pageName=productUsed&useyn=${list.useyn}&pageNum2=1">수정</a>
							</td>
							<c:choose>
