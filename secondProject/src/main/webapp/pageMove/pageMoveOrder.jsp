...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
		...
		<table border=0 width=1200 align=center>
				...
				<c:if test="${startPage > pageBlock }">
					<a href="javascript:callFunction2(${startPage - pageBlock },'order');">[이전]</a>
				</c:if>
				</td>
...
<%-- 							<a href="javascript:callFunction('mileage','new',${i });"><span style="padding:0px 4px;"><font color=red><b>${i }</b></font></span></a> --%>
							<a href="javascript:callFunction2(${i },'order');"><span style="padding:0px 4px;"><font color=red><b>${i }</b></font></span></a>
						</c:when>
						<c:otherwise>
...
<%-- 							<a href="javascript:callFunction('mileage','new',${i });"><span style="padding:0px 4px;">${i }</span></a> --%>
							<a href="javascript:callFunction2(${i },'order');"><span style="padding:0px 4px;">${i }</span></a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				</td>			
				...
				<td width=75 style="text-align:center">
				<c:if test="${endPage < pageCount }">
					<a href="javascript:callFunction2(${startPage + pageBlock },'order');">[다음]</a>
				</c:if>
				</td>
			</c:if>
				<tr>
		...
		<tr height=45>
			...
			<td width=240 style="text-align:center;" id="tdcolor">수령인</td>
			<td width=240 style="text-align:center;" id="tdcolor">주문금액</td>
			<td width=240 style="text-align:center;" id="tdcolor">적립포인트</td>
		...
		<tr height=80>
			...
			<td width=240 style="text-align:center;border-bottom:1px solid #5055B1;">&nbsp;&nbsp;<fmt:formatNumber value="${list.od_price }"/></td>
 			<td width=240 style="text-align:center;border-bottom:1px solid #5055B1;">&nbsp;&nbsp;<fmt:formatNumber value="${list.od_point }"/></td>
 			...
 			<td width=240 style="text-align:center;border-bottom:1px solid #5055B1;">&nbsp;&nbsp;<a href="/order/orderViewDetail?od_id=${list.od_id }&od_point=${list.od_point }&mb_id=${list.mb_id }&order_option=a&pageNum=${pageNum}&pageName=order&option2=new">상세보기</a></td>
 			<td width=240 style="text-align:center;border-bottom:1px solid #5055B1;">&nbsp;&nbsp;<c:if test="${list.od_status eq '주문' }"><a href="/order/cancel?od_id=${list.od_id }&order_option=myPage">주문취소</a></c:if></td>
