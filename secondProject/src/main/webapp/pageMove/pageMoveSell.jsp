...
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
		...
		<table border=0 width=1200 align=center>
				...
				<c:if test="${startPage > pageBlock }">
					<a href="javascript:callFunction4(${startPage - pageBlock },'sell');">[이전]</a>
				</c:if>
				</td>
...
<%-- 							<a href="javascript:callFunction('mileage','new',${i });"><span style="padding:0px 4px;"><font color=red><b>${i }</b></font></span></a> --%>
							<a href="javascript:callFunction4(${i },'sell');"><span style="padding:0px 4px;"><font color=red><b>${i }</b></font></span></a>
						</c:when>
						<c:otherwise>
...
<%-- 							<a href="javascript:callFunction('mileage','new',${i });"><span style="padding:0px 4px;">${i }</span></a> --%>
							<a href="javascript:callFunction4(${i },'sell');"><span style="padding:0px 4px;">${i }</span></a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				</td>			
				...
				<td width=75 style="text-align:center">
				<c:if test="${endPage < pageCount }">
					<a href="javascript:callFunction4(${startPage + pageBlock },'sell');">[다음]</a>
				</c:if>
				</td>
			</c:if>
				<tr>
...
<table width=1200 border=0>
		...
		<c:set var="i" value="${0 }"/>
		<c:forEach var="lists" items="${v3 }">		
		<tr height=80>
			<td width=240 style="text-align:center;border-bottom:1px solid #5055B1;"><img src="/upload/${lists.image }" style="width:80px;height:80px;vertical-align:middle;"></td>
